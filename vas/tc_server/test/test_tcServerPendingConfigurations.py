# vFabric Administration Server API
# Copyright (c) 2012 VMware, Inc. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.


import re
from unittest.case import TestCase
from vas.shared.Security import Security
from vas.tc_server.TcServerPendingConfiguration import TcServerPendingConfiguration
from vas.tc_server.TcServerPendingConfigurations import TcServerPendingConfigurations
from vas.test.StubClient import StubClient

class TestTcServerPendingConfigurations(TestCase):
    __client = StubClient()

    def setUp(self):
        self.__client.delegate.reset_mock()
        self.__pending_configurations = TcServerPendingConfigurations(self.__client,
            'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/')

    def test_delete(self):
        self.__pending_configurations.delete(
            TcServerPendingConfiguration(self.__client,
                'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/2/'))
        self.__client.delegate.delete.assert_called_once_with(
            'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/2/')

    def test_create(self):
        self.__client.delegate.post_multipart.return_value = 'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/2/'

        pending_configuration = self.__pending_configurations.create('conf/server.xml', 'configuration-content')

        self.assertIsInstance(pending_configuration, TcServerPendingConfiguration)
        self.__client.delegate.post_multipart.assert_called_once_with(
            'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/', 'configuration-content',
                {'path': 'conf/server.xml'})

    def test_attributes(self):
        self.assertIsInstance(self.__pending_configurations.security, Security)

    def test__create_item(self):
        self.assertIsInstance(
            self.__pending_configurations._create_item(self.__client,
                'https://localhost:8443/tc-server/v1/groups/0/instances/1/configurations/pending/2/'),
            TcServerPendingConfiguration)

    def test___iter__(self):
        count = 0
        #noinspection PyUnusedLocal
        for node in self.__pending_configurations:
            count += 1

        self.assertEqual(3, count)

    def test_repr(self):
        self.assertIsNone(re.match('<.* object at 0x.*>', repr(self.__pending_configurations)),
            '__repr__ method has not been specified')
        eval(repr(self.__pending_configurations))
