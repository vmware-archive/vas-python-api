...
class TestTcServerInstallations(TestCase):
    __client = StubClient()

    def setUp(self):
        self.__client.delegate.reset_mock()
        self.__installations = TcServerInstallations(self.__client,
    ...
    def test_create(self):
        self.__client.delegate.post.return_value = 'https://localhost:8443/tc-server/v1/groups/1/installations/2/'

        group = self.__installations.create(
            InstallationImage(self.__client, 'https://localhost:8443/tc-server/v1/installation-images/0/'))

        self.assertIsInstance(group, TcServerInstallation)
        self.__client.delegate.post.assert_called_once_with(
            'https://localhost:8443/tc-server/v1/groups/0/installations/',
                {'image': 'https://localhost:8443/tc-server/v1/installation-images/0/'}, 'installation')
    ...
    def test__create_item(self):
        self.assertIsInstance(
            self.__installations._create_item(self.__client,
                'https://localhost:8443/tc-server/v1/groups/1/installations/2/'), TcServerInstallation)
    ...
    def test_repr(self):
        self.assertIsNone(re.match('<.* object at 0x.*>', repr(self.__installations)), '__repr__ method has not been specified')
