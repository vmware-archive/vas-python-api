...
class TestInstallationImages(TestCase):
    ...
    def setUp(self):
        ...
        self.__installation_images = TcServerInstallationImages(self.__client,
            'https://localhost:8443/tc-server/v1/installation-images/')

    def test_delete(self):
        self.__installation_images.delete(
            TcServerInstallationImage(self.__client, 'https://localhost:8443/tc-server/v1/installation-images/0/'))
        self.__client.delegate.delete.assert_called_once_with(
            'https://localhost:8443/tc-server/v1/installation-images/0/')

    def test_create(self):
        self.__client.delegate.post_multipart.return_value = 'https://localhost:8443/tc-server/v1/installation-images/0/'

        installation_image = self.__installation_images.create('2.8.0.RELEASE',
            '/tmp/vfabric-tc-server-standard-2.8.0.RELEASE.zip')
    ...
    def test__create_item(self):
        self.assertIsInstance(
            self.__installation_images._create_item(self.__client,
                'https://localhost:8443/tc-server/v1/installation-images/0/'), TcServerInstallationImage)
