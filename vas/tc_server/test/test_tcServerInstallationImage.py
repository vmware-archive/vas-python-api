...
class TestInstallationImage(TestCase):
    ...
    def setUp(self):
        ...
        self.__installation_image = TcServerInstallationImage(self.__client,
            'https://localhost:8443/tc-server/v1/installation-images/0/')

    def test_attributes(self):
        self.assertEqual(7340032, self.__installation_image.size)
        self.assertEqual('2.7.0.RELEASE', self.__installation_image.version)
        self.assertEqual(
            [TcServerInstallation(self.__client, 'https://localhost:8443/tc-server/v1/groups/1/installations/2/'),
             TcServerInstallation(self.__client, 'https://localhost:8443/tc-server/v1/groups/3/installations/4/')],
            self.__installation_image.installations)
        self.assertIsInstance(self.__installation_image.security, Security)

    def test_repr(self):
        self.assertIsNone(re.match('<.* object at 0x.*>', repr(self.__installation_image)), '__repr__ method has not been specified')
