...
from vas.sqlfire.AgentInstances import AgentInstances, AgentInstance
...
class TestAgentInstances(VasTestCase):
    def test_list(self):
        self._assert_collection(
            AgentInstances(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/'))
    ...
    def test_create(self):
        ...
        self._return_location('https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/')
        ...
        self._assert_post(location,
            {'name': 'example', 'installation': installation_location, 'jvm-options': ['-Da=alpha']},
            'agent-group-instance')

    def test_detail(self):
        self._assert_item(
            AgentInstance(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/'), [
                ...
                ('node_instances', [
                    AgentNodeInstance(self._client, 'https://localhost:8443/sqlfire/v1/nodes/0/agent-instances/5/'),
                    AgentNodeInstance(self._client, 'https://localhost:8443/sqlfire/v1/nodes/1/agent-instances/6/')
                ]),
                ('pending_configurations', lambda actual: self.assertIsInstance(actual, AgentPendingConfigurations))
            ])

    def test_start_parallel(self):
        AgentInstance(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/').start()
        self._assert_post('https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/state/',
            {'status': 'STARTED', 'serial': False})

    def test_start_serial(self):
        AgentInstance(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/').start(True)
        self._assert_post('https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/state/',
            {'status': 'STARTED', 'serial': True})

    def test_stop_parallel(self):
        AgentInstance(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/').stop()
        self._assert_post('https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/state/',
            {'status': 'STOPPED', 'serial': False})

    def test_stop_serial(self):
        AgentInstance(self._client, 'https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/').stop(True)
        self._assert_post('https://localhost:8443/sqlfire/v1/groups/2/agent-instances/4/state/',
    ...
    def test_update_all_optionals(self):
        ...
        AgentInstance(self._client, location).update(Installation(self._client, installation_location), ['-Da=alpha'])

        self._assert_post(location, {'installation': installation_location, 'jvm-options': ['-Da=alpha']})
