...
class TestRabbitMqLiveConfiguration(TestCase):
    __client = StubClient()

    def setUp(self):
        self.__client.delegate.reset_mock()
        self.__configuration = RabbitMqLiveConfiguration(self.__client,
            'https://localhost:8443/rabbitmq/v1/groups/0/instances/1/configurations/live/2/')

    def test_attributes(self):
        self.assertEqual('conf/rabbit.config', self.__configuration.path)
