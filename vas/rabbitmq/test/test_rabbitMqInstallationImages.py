...
class TestInstallationImages(TestCase):
    ...
    def setUp(self):
        ...
        self.__installation_images = RabbitMqInstallationImages(self.__client,
            'https://localhost:8443/rabbitmq/v1/installation-images/')

    def test_delete(self):
        self.__installation_images.delete(
            RabbitMqInstallationImage(self.__client, 'https://localhost:8443/rabbitmq/v1/installation-images/0/'))
        self.__client.delegate.delete.assert_called_once_with(
            'https://localhost:8443/rabbitmq/v1/installation-images/0/')

    def test_create(self):
        self.__client.delegate.post_multipart.return_value = 'https://localhost:8443/rabbitmq/v1/installation-images/0/'

        installation_image = self.__installation_images.create('2.8.0.RELEASE',
            '/tmp/vfabric-rabbitmq-standard-2.8.0.RELEASE.zip')

        self.assertIsInstance(installation_image, RabbitMqInstallationImage)
        self.__client.delegate.post_multipart.assert_called_once_with(
            'https://localhost:8443/rabbitmq/v1/installation-images/',
            '/tmp/vfabric-rabbitmq-standard-2.8.0.RELEASE.zip', {'version': '2.8.0.RELEASE'})
    ...
    def test__create_item(self):
        self.assertIsInstance(
            self.__installation_images._create_item(self.__client,
                'https://localhost:8443/rabbitmq/v1/installation-images/0/'), RabbitMqInstallationImage)
