...
class TestRabbitMqLog(TestCase):
    __client = StubClient()

    def setUp(self):
        self.__client.delegate.reset_mock()
        self.__log = RabbitMqLog(self.__client, 'https://localhost:8443/rabbitmq/v1/nodes/0/instances/3/logs/4/')

    def test_attributes(self):
        self.assertEqual(datetime(2012, 5, 24, 15, 20, 56), self.__log.last_modified)
        self.assertEqual('rabbit.log', self.__log.name)
        self.assertEqual(17638, self.__log.size)
        self.assertIsInstance(self.__log.instance, RabbitMqNodeInstance)
    ...
    def test_content(self):
        ...
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/rabbitmq/v1/nodes/0/instances/3/logs/4/content/')

        self.__client.delegate.reset_mock()
        self.assertEqual('nodes-instances-logs-content\n', self.__log.content(start_line='1'))
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/rabbitmq/v1/nodes/0/instances/3/logs/4/content/?start-line=1')

        self.__client.delegate.reset_mock()
        self.assertEqual('nodes-instances-logs-content\n', self.__log.content(end_line='2'))
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/rabbitmq/v1/nodes/0/instances/3/logs/4/content/?end-line=2')
