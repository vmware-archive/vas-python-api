...
class TestGemFireAgentLog(TestCase):
    __client = StubClient()

    def setUp(self):
        self.__client.delegate.reset_mock()
        self.__log = GemFireAgentLog(self.__client,
            'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/')
    ...
    def test_attributes(self):
        ...
        self.assertIsInstance(self.__log.instance, GemFireAgentNodeInstance)
    ...
    def test_content(self):
        ...
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/content/')

        self.__client.delegate.reset_mock()
        self.assertEqual('nodes-agent-instances-logs-content\n', self.__log.content(start_line='1'))
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/content/?start-line=1')

        self.__client.delegate.reset_mock()
        self.assertEqual('nodes-agent-instances-logs-content\n', self.__log.content(end_line='2'))
        self.__client.delegate.get.assert_called_once_with(
            'https://localhost:8443/gemfire/v1/nodes/0/agent-instances/3/logs/4/content/?end-line=2')
